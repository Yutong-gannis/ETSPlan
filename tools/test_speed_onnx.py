...
if __name__ == '__main__':
    total_time = 0
    batch = 1
    session = onnxruntime.InferenceSession('./etsplan.onnx', providers=['CUDAExecutionProvider',
    ...
    output2_name = session.get_outputs()[2].name
    ...
    for i in range(1000):
        ...
        actions = np.zeros((batch, 10, 2), dtype=np.float32)
        ...
        pred_cls, trajectory, feature = session.run([output0_name,
                                                    output1_name,
                                                    output2_name,],
                                                    {input0_name: img,
