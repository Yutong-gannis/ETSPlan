...
class Trainer_3(Trainer):
    def __init__(self, args):
        self.stage = 3
        super(Trainer_3, self).__init__(args)
    ...
    def train_step(self, epoch, batch, num_steps):
        front_sequence, leftrear_sequence, rightrear_sequence, nav_sequence, trajectory_sequence = batch
        #front_sequence, leftrear_sequence, rightrear_sequence, nav_sequence, trajectory_sequence = front_sequence.to(self.device), leftrear_sequence.to(self.device), rightrear_sequence.to(self.device), nav_sequence.to(self.device), trajectory_sequence.to(self.device)
        ...
        for t in range(seq_length):
            num_steps += 1
            front, leftrear, rightrear, nav, trajectory_label = front_sequence[:, t, :, :, :].to(self.device).float(), leftrear_sequence[:, t, :, :, :].to(self.device).float(), rightrear_sequence[:, t, :, :, :], nav_sequence[:, t, :, :, :].to(self.device).float(), trajectory_sequence[:, t, :, :].to(self.device).float()
            pred_cls, pred_trajectory, feature = self.model(front, leftrear, rightrear, nav, self.hist_feature, self.hist_trajectory, self.speed_limit, self.stop, self.traffic_convention)
    ...
    def validation_step(self, epoch, num_steps):
        ...
        with torch.no_grad():
            for data in tqdm(self.val_loader):
                front_sequence, leftrear_sequence, rightrear_sequence, nav_sequence, trajectory_sequence = data
                #front_sequence, leftrear_sequence, rightrear_sequence, nav_sequence, trajectory_sequence = front_sequence.to(self.device), leftrear_sequence.to(self.device), rightrear_sequence.to(self.device), nav_sequence.to(self.device), trajectory_sequence.to(self.device)
                seq_length = front_sequence.size(1)
