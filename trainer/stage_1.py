...
class Trainer_1(Trainer):
    def __init__(self, args):
        self.stage = 1
        super(Trainer_1, self).__init__(args)
        ...
        self.nav = torch.FloatTensor(np.zeros((self.batch_size, 3, 64, 64)), device=self.device)
    ...
    def train_step(self, epoch, batch, num_steps):
        front_sequence, trajectory_sequence = batch
        #front_sequence, trajectory_sequence = front_sequence.to(self.device), trajectory_sequence.to(self.device)
        seq_length = front_sequence.size(1)
        hist_feature = torch.zeros((self.batch_size, 40, 128)).to(self.device)
        total_loss = 0
        for t in range(seq_length):
            num_steps += 1
            front, trajectory_label = front_sequence[:, t, :, :, :].to(self.device).float(), trajectory_sequence[:, t, :, :].to(self.device).float()
            pred_cls, pred_trajectory, feature = self.model(front, self.leftrear, self.rightrear, self.nav, self.hist_feature, self.hist_trajectory, self.speed_limit, self.stop, self.traffic_convention)
    ...
    def validation_step(self, epoch, num_steps):
        ...
        with torch.no_grad():
            for data in tqdm(self.val_loader):
                front_sequence, trajectory_sequence = data
                #front_sequence, trajectory_sequence = front_sequence.to(self.device), trajectory_sequence.to(self.device)
                seq_length = front_sequence.size(1)
                ...
                logger.info("val | val_loss: {}", val_loss)
        self.model.train()
